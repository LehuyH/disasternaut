...
export function allowedToBuild(buildingName:string):(boolean|string|null)[] {
    ...
    //No collisions
    if(isTouchingBuilding(preview)) return [false, "There is not enough room to build this here."]
...
function isTouchingBuilding(preview:Character<any>): boolean {
    let touch = false
    k.every("building",building=>{
        const touchCheck = isTouching(preview.worldArea(),building.worldArea())
